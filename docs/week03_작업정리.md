...
## reminder/todo 답변 포맷 미적용 버그와 프롬프트 보강

### 증상

일정 조회는 "- [제목] MM/DD HH:MM ~ HH:MM (참여자: ...)" 한 줄 포맷을 잘 따르는데,
알림·할 일 조회는 포맷 없이 자유 형식으로 출력됐다. (1차 PR KPT의 Try 항목)

### 원인 (코드 버그가 아니라 프롬프트 스코프 문제)

프롬프트 스택(Week 1 → 2 → 3 누적) 안에서 한 줄 포맷 규칙이 reminder/todo에는 도달하지 않았다.

1. Week 1 원문이 "사용자에게 **일정**을 보여줄 때는" — 처음부터 '일정' 전용 스코프.
2. Week 2의 "[Week 1 답변 규칙 무효화]" 조각이 포맷 규칙을 통째로 끄고, Week 3 스택에도 그대로 남아 있다.
3. Week 3의 재활성화 문구("일정을 보여줄 때는 Week 1의 일정 한 줄 포맷을 사용한다")도 '일정'만 언급.

결국 schedule은 "무효화 → 일정에 한해 재활성화"로 포맷이 적용되지만,
reminder/todo는 **적용되는 형식 지시가 0개**였다. 지시를 "안 따른" 게 아니라 따를 지시가 없던 상태.

부차적 요인: 한 줄 포맷 자체가 시작~종료 시간+참여자 전제라 todo(마감일·priority 중심),
reminder(참여자 없음)에 그대로 못 맞고, `list_saved_requests`는 `SELECT *`라
`raw_json`·`created_at` 같은 내부 컬럼까지 모델에 노출돼 자유 요약 유인이 컸다.

### 대응 (프롬프트 보강)

`WEEK03_ANSWER_FORMAT_PROMPT`(**[Week 3 답변 포맷]**)를 새로 추가하고 `week03_prompt_parts()`에 누적했다.

- Week 2의 "[Week 1 답변 규칙 무효화]"는 Week 3에서는 적용하지 않는다고 명시 (모든 kind).
- kind별 한 줄 포맷 정의:
  - 일정: Week 1 포맷 그대로 `- [제목] MM/DD HH:MM ~ HH:MM (참여자: ...)`
  - 알림: `- [제목] MM/DD HH:MM (알림)` — 시간 없으면 HH:MM 생략
  - 할 일: `- [제목] ~MM/DD (우선순위: 값)` — 마감일/priority 없으면 해당 부분 생략
- `request_id`·`raw_json`·`created_at` 같은 내부 필드는 표기 금지, 여러 건은 줄바꿈 나열.
- "[Week 2 → Week 3 연결]" 조각의 "일정을 보여줄 때는 Week 1 한 줄 포맷" 문구도
  "[Week 3 답변 포맷]의 kind별 한 줄 포맷"을 가리키도록 수정했다.

> 남은 한계: 조회 라우팅과 마찬가지로 프롬프트 의존이다. 코드 레벨 방어가 필요하면
> `list_saved_requests` 반환에서 `raw_json` 등 내부 컬럼을 제외해 모델이 보는 row를
> 일정 조회 수준으로 정리하는 보강이 있다(이번 수정 범위에서는 제외).

### 점검하며 확인한 다른 한계 (이번 수정 범위 밖)

- **reminder/todo 수정·삭제 경로가 없다** — `personal_update_saved_schedule`/`personal_delete_saved_schedules`는
  `schedules` 테이블만 다루고, `todos`/`reminders`를 고치거나 지우는 tool이 없다.
  프롬프트 5번 규칙도 일정 전용이라 "알림 지워줘"는 처리 불가. 2차 PR 후보.
- **group_schedule 기본 조회 제외** — `personal_list_saved_schedules`의 기본 `kind`가
  `personal_schedule`이라 "내 일정 보여줘"에 그룹 일정이 빠질 수 있다(명시적으로 kind를 넘겨야 조회됨).
