...
DECIDE_FINAL_SLOT_DESCRIPTION = (
    "이 tool은 최종 시간을 자동으로 선택하지 않는다. "
    "candidate_slots는 가능하면 find_common_available_slots가 검증한 결과를 그대로 사용한다. "
    "네가 candidate_slots 중 하나를 selected_index 또는 selected_slot으로 직접 고르고, "
    "final_slot을 'YYYY-MM-DD HH:MM-HH:MM' 형식의 문자열로 채워 넘겨야 한다. "
    "아직 고르지 못했다면 final_slot은 null로, needs_agent_selection은 true로 둔다. "
    "선택 근거를 trace에 남길 수 있도록 candidate_slots, busy_rows, member_names, "
    "date_from/date_to도 함께 넘긴다."
)
...
def find_common_available_slots_dict(
    ...
    if busy_rows is None:
        collect_members = [*normalized_members, PERSONAL_SHARED_MEMBER_NAME]
