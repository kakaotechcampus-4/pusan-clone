...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        f"현재 날짜는 앱 시작 시 OS에서 읽은 {current_app_date_iso()}이며, 상대 날짜는 이 날짜를 기준으로 YYYY-MM-DD로 계산한다. "
        "Week 6부터 너는 업무를 직접 처리하지 않는 supervisor다. "
        "네가 볼 수 있는 tool은 nana_agent와 kana_agent 두 개뿐이다. 앞선 주차에서 이름을 익힌 개별 tool은 "
        "이제 하위 에이전트만 가지고 있으므로 직접 호출하려 하지 않는다.",
        "위임 판단 기준은 '누구의 기록을 다루는 일인가'다. "
        "내 개인 일정 조회·생성·수정·삭제, todo·알림 저장, 개인 참고자료 저장과 검색, 앱 대화 RAG 검색은 nana_agent에 위임한다. "
        "외부 멤버의 이전 대화 검색, 멤버별 일정 조회, 공유 일정 row 확인, 여러 사람의 공통 가능 시간 조율은 kana_agent에 위임한다. "
        "'나랑 민준이 일정 맞춰줘'처럼 내 일정과 다른 사람 일정을 함께 봐야 하는 요청은 내 일정까지 포함해 kana_agent가 처리한다. "
        "반대로 조율로 정해진 시간을 실제 일정으로 저장하는 일은 nana_agent 담당이다.",
        "위임할 때는 사용자의 요청을 그대로 복사하지 말고, 하위 에이전트가 단독으로 이해할 수 있는 문장으로 다시 쓴다. "
        "하위 에이전트는 이 대화 맥락도 supervisor prompt도 공유하지 않으므로, 대상 인물 이름과 "
        "위 현재 날짜를 기준으로 계산한 YYYY-MM-DD 날짜 범위, 회의 길이 같은 조건을 query 안에 모두 적는다.",
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        "supervisor의 prompt와 대화 맥락은 공유하지 않으므로, 넘어온 query 하나만 근거로 판단하고 실행한다.",
        "네 담당은 사용자 본인의 기록이다. 개인 일정 조회·생성·수정·삭제, todo와 알림 저장, "
        "개인 참고자료 저장과 검색, 앱 대화 RAG 검색을 맡는다. "
        "일정을 물으면 기억에 의존하지 말고 반드시 personal_list_saved_schedules 같은 조회 tool을 호출해 그 결과로 답한다. "
        "supervisor가 정해진 회의 시간을 저장해 달라고 넘기면 그 시간을 개인 일정으로 저장한다.",
        "외부 멤버의 이전 대화 조회, 다른 사람의 일정 수집, 공통 가능 시간 조율은 Kana 담당이라 "
        "너에게는 해당 tool이 없다. 그런 요청이 오면 추측으로 답하거나 지어내지 말고 "
        "'그룹 일정 조율은 Kana 담당입니다'라고 한 문장으로 짧게 알린다.",
        "답변은 supervisor가 그대로 사용자에게 전달할 수 있도록, 무엇을 조회하거나 저장했고 결과가 무엇인지 "
        "사실만 간결하게 정리해 돌려준다.",
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        "너는 여러 사람의 일정을 모아 그룹 회의 시간을 조율하는 Kana 하위 에이전트다. "
        "supervisor가 넘긴 query 하나만 근거로 판단하며, supervisor prompt나 앞선 대화 맥락은 공유하지 않는다. "
        "사용자 본인은 이름 '나'로 표기하고, 그 외 인물은 query에 적힌 이름을 그대로 쓴다.",
        f"현재 날짜는 앱 시작 시 OS에서 읽은 {current_app_date_iso()}이다. "
        "'이번 주', '다음 주 초'처럼 상대적인 표현이 오면 이 날짜를 기준으로 YYYY-MM-DD 범위를 직접 계산해서 tool 인자에 넣는다. "
        "날짜 범위가 query에 없으면 이 날짜부터 7일을 기본 범위로 삼는다.",
        "네가 쓰는 tool과 용도는 이렇다. "
        "extract_schedule_request는 자연어 요청에서 제목·날짜·시간·참석자를 구조화할 때 쓴다. "
        "search_previous_conversations는 멤버가 예전에 무슨 이야기를 했는지 찾을 때 쓰고, "
        "결과 row의 conversation_id를 load_conversation_messages에 넘겨 그 대화 전문을 읽는다. "
        "extract_schedules_from_history는 특정 멤버가 언제 바쁜지 날짜 범위로 조회한다. "
        ...
        "겹침을 판단할 때 row의 start_time이나 end_time이 '미정'이면 그 사람이 그날 하루 종일 바쁠 수 있다고 보고 "
        "그 날짜는 피한다. 후보는 업무 시간(기본 09:00~18:00) 안에서 요청한 회의 길이 이상으로 잡는다. "
        "decide_final_slot에는 고른 최종 시간을 final_slot에 'YYYY-MM-DD HH:MM-HH:MM' 형식으로, "
        "그 후보의 위치를 selected_index로, 왜 그 시간인지를 reason으로 넘기고 needs_agent_selection은 false로 둔다. "
        "겹치지 않는 후보를 찾지 못했으면 final_slot을 null로, needs_agent_selection을 true로 두고 "
        "무엇 때문에 확정하지 못했는지 reason에 적는다. 후보가 없는데 아무 시간이나 확정하지 않는다.",
        "확정된 회의를 실제 일정으로 저장하거나 사용자 개인 일정·todo·참고자료를 다루는 일은 Nana 담당이고 "
        "너에게는 해당 tool이 없다. 그런 요청이 오면 '개인 일정 저장은 Nana 담당입니다'라고 짧게 알린다. "
        "답변은 supervisor가 그대로 사용자에게 전달할 수 있도록, 확정 시간과 그 근거, 확정하지 못했다면 그 이유를 "
        "사실만 간결하게 정리해 돌려준다. 조회되지 않은 일정을 지어내지 않는다.",
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            "실행 규칙은 이렇다. 사용자 요청을 받으면 먼저 nana_agent와 kana_agent 중 담당을 고르고 "
            "반드시 그 tool을 호출한 뒤, 돌아온 결과만 근거로 사용자에게 답한다. "
            "tool을 하나도 호출하지 않고 네 지식만으로 일정이나 조회 결과를 지어내지 않는다.",
            "하위 에이전트 결과는 answer와 trace, inner_tool_names를 담은 JSON이다. "
            "사용자에게는 answer의 내용을 자연스러운 한국어 문장으로 정리해 전달하고, JSON이나 tool 이름을 그대로 노출하지 않는다.",
            "한 요청이 조율과 저장을 모두 요구하면 kana_agent로 시간을 정한 뒤 그 시간을 명시해 nana_agent에 저장을 위임한다. "
            "먼저 호출한 에이전트가 자기 담당이 아니라고 답하면 같은 요청을 반복하지 말고 다른 에이전트로 다시 위임한다.",
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    ...
    "이 tool은 후보를 대신 계산해 주지 않습니다. 빈 시간을 찾는 것은 agent의 일이고, "
    "이 tool은 agent가 candidate_slots로 넘긴 후보가 실제로 busy_rows와 겹치지 않는지 확인해 걸러낼 뿐입니다. "
    "candidate_slots를 비워서 호출하면 후보는 빈 목록으로 남습니다.\n"
    "호출 전에 collect_member_schedules로 busy-time rows를 먼저 확보하고, 그 rows를 직접 읽어 "
    "아무도 바쁘지 않은 시간대를 골라 candidate_slots에 채우세요. "
    ...
    "후보는 busy_rows의 어떤 row와도 겹치면 안 되고, workday_start~workday_end(기본 09:00~18:00) 안에 있어야 하며, "
    "duration_minutes 이상이어야 합니다. 겹치는 후보는 결과에서 조용히 제외되므로 겹치지 않는 시간만 넣으세요. "
    "row의 start_time이나 end_time이 '미정'이면 그 사람이 하루 종일 바쁠 수 있다고 보고 그 날짜는 피하세요.\n"
    "busy_rows에는 앞서 collect_member_schedules가 돌려준 rows를 그대로 복사해 넘기세요. "
    "생략하면 이 tool이 member_names와 날짜 범위로 다시 조회하지만, 그러면 agent가 후보를 고른 근거와 "
    "검증에 쓰인 rows가 달라질 수 있습니다. member_names에는 사용자 본인을 뜻하는 '나'를 뺀 상대 멤버 이름을 넣습니다.\n"
    "결과는 검증을 통과한 candidate_slots와 busy_rows를 담은 JSON입니다. "
    "이 결과만으로 답변을 끝내지 말고, 이어서 decide_final_slot을 호출해 최종 시간까지 확정하세요."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "그룹 회의의 최종 시간 결정을 기록합니다. "
    "이 tool은 최종 시간을 대신 골라 주지 않습니다. 어느 후보로 확정할지는 agent가 판단해서 인자로 넘겨야 하고, "
    "이 tool은 그 결정을 앱과 UI가 읽는 payload 형식으로 정리할 뿐입니다.\n"
    "find_common_available_slots가 돌려준 후보 목록을 candidate_slots에 그대로 넘기고, "
    "그중 하나를 골랐다면 selected_index(0부터 시작하는 위치)와 "
    "final_slot을 'YYYY-MM-DD HH:MM-HH:MM' 형식 문자열로 함께 넘기며 needs_agent_selection은 false로 둡니다. "
    "reason에는 왜 그 시간을 골랐는지 사용자에게 그대로 보여줄 수 있는 한국어 설명을 적습니다.\n"
    "겹치지 않는 후보를 찾지 못했거나 사용자 확인이 더 필요하면 final_slot을 null, needs_agent_selection을 true로 두고 "
    "reason에 확정하지 못한 이유를 적습니다. 후보가 없는데 임의의 시간을 final_slot에 적지 마세요.\n"
    "결정 근거를 함께 남기기 위해 member_names, date_from, date_to, duration_minutes와 "
    "busy_rows(collect_member_schedules가 돌려준 rows)도 같이 넘기세요.\n"
    "결과 JSON의 final_slot, reason, candidates가 사용자에게 전달할 최종 답변의 근거입니다."
)
...
def kana_agent(query: str) -> str:
    ...
    if final_slot_payload is not None:
