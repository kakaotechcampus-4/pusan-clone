...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        "당신은 supervisor입니다. 위에 나온 tool들은 당신이 직접 쓰는 게 아니라 "
        "nana_agent 또는 kana_agent 하위 에이전트가 대신 사용합니다. 당신은 직접 일정을 만들거나 검색하지 않고, "
        "요청을 알맞은 하위 에이전트에게 위임하기만 합니다.",
        "개인적인 일(내 일정 생성/조회/수정/삭제, 할 일/알림 저장, 개인 참고자료, 지난 대화 기록 검색)은 nana_agent에게 위임하세요.",
        "다른 사람이 관련된 일(외부 멤버의 이전 대화/일정 조회, 공유 일정 확인, 여러 사람의 공통 가능 시간 찾기·확정)은 kana_agent에게 위임하세요.",
        "요청이 모호하면 '누구랑'처럼 다른 사람이 언급됐는지로 판단하세요. 언급이 없으면 nana_agent, 있으면 kana_agent입니다.",
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        "당신은 이제 Nana 하위 에이전트로서 supervisor가 위임한 개인 업무만 처리합니다. "
        "그룹 일정 조율이나 외부 멤버의 일정/대화를 확인해야 하는 요청이 오면, "
        "그건 당신 담당이 아니라고 짧게 알리세요.",
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        f"당신은 Kana입니다. 현재 시스템 날짜는 {current_app_date_iso()}입니다. 모든 날짜 표현은 이 날짜를 기준으로 계산하세요.",
        "당신은 외부 멤버의 이전 대화, 공유 일정 조회, 그리고 여러 사람의 공통 가능 시간을 찾고 확정하는 일을 담당합니다.",
        "멤버가 실제로 한 말을 키워드로 찾으려면 search_previous_conversations를, 특정 대화 전체를 읽으려면 "
        "그 결과의 conversation_id로 load_conversation_messages를 사용하세요.",
        "멤버들의 바쁜 시간/일정만 필요하면 extract_schedules_from_history를, 이미 공유 일정 저장소에 등록된 일정 "
        "자체를 확인하려면 list_shared_schedules를, 내 일정과 여러 멤버의 일정을 한 번에 모아 비교해야 하면 "
        "collect_member_schedules를 사용하세요.",
        "collect_member_schedules로 모은 busy_rows를 근거로 겹치지 않는 후보를 직접 골라 "
        "find_common_available_slots를 호출하고, 이어서 그 후보 중 최종 시간을 골라 decide_final_slot을 "
        "호출하세요. 이 두 tool은 계산을 대신 해주지 않으므로 후보/최종 시간은 항상 당신이 직접 채웁니다.",
        "확정된 일정을 실제로 저장하는 것은 당신의 담당이 아닙니다 — 최종 시간이 정해지면 그 정보를 답변에 담아 "
        "supervisor에게 전달하세요. 저장은 Nana가 처리합니다.",
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            "직접 답변을 만들지 말고, 반드시 nana_agent 또는 kana_agent 중 하나를 호출한 뒤 "
            "그 결과(answer 또는 final_decision_payload)만 근거로 사용자에게 답하세요.",
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "이 tool은 공통 가능 시간을 스스로 계산하지 않습니다. 먼저 collect_member_schedules로 "
    "모은 busy_rows를 근거로, 그 rows와 겹치지 않는 시간대를 당신이 직접 골라 candidate_slots로 "
    "넘기세요. candidate_slots의 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), "
    "duration_minutes(분), reason(이 시간을 고른 짧은 이유)을 포함해야 합니다. "
    "요청한 date_from~date_to 범위 밖이거나, workday_start~workday_end 밖이거나, "
    "duration_minutes보다 짧거나, busy_rows와 겹치는 후보는 조용히 걸러지고 결과에 남지 않습니다. "
    "이 tool의 결과만으로 답변을 끝내지 말고, 검증된 candidate_slots를 가지고 이어서 "
    "decide_final_slot을 호출해 최종 시간을 확정하세요."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "이 tool도 최종 시간을 스스로 고르지 않습니다. find_common_available_slots가 검증한 "
    "candidate_slots 중 하나를 selected_index(후보 순번) 또는 selected_slot으로 넘기거나, "
    "이미 'YYYY-MM-DD HH:MM-HH:MM' 형식으로 정리한 문자열을 final_slot에 직접 넣으세요. "
    "아직 사용자 확인이 더 필요하거나 후보 중 무엇을 고를지 판단이 서지 않으면 "
    "final_slot과 selected_index/selected_slot을 비워두세요 — 그러면 needs_agent_selection이 "
    "자동으로 true로 유지됩니다. reason에는 이 시간을 고른(또는 아직 확정하지 못한) 이유를 "
    "짧게 남기고, candidate_slots/busy_rows/member_names/date_from/date_to도 함께 넘겨 "
    "최종 결정의 근거가 trace에 남게 하세요."
)
...
def find_common_available_slots_dict(
    ...
    if busy_rows is None:
        ...
        if not collect_result.get("ok"):
            raise RuntimeError(collect_result.get("error") or "내 일정/외부 멤버 일정을 모으지 못했습니다.")
...
def find_common_available_slots(
    ...
    try:
        ...
        return json.dumps({"ok": False, "tool_name": "find_common_available_slots", "error": str(exc)}, ensure_ascii=False)
...
def nana_agent(query: str) -> str:
    ...
    try:
        ...
        return json.dumps({"ok": False, "tool_name": "nana_agent", "error": str(exc)}, ensure_ascii=False)
...
def kana_agent(query: str) -> str:
    ...
    try:
        ...
        return json.dumps({"ok": False, "tool_name": "kana_agent", "error": str(exc)}, ensure_ascii=False)
    ...
    final_decision_payload = (
        final_slot_payload if final_slot_payload and final_slot_payload.get("final_slot") else None
    )
