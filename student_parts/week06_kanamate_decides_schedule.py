...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        ...
        (
            "지금부터 너는 카나메이트 supervisor다. 위에 적힌 주차별 규칙은 하위 agent들이 지키는 것이고, "
            ...
            "개인 일정과의 충돌 판단은 하위 agent가 아니라 supervisor인 네 몫이다."
        )
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        ...
        (
            ...
            "특정 멤버들의 바쁜 시간만 필요하면 extract_schedules_from_history를 쓰고, "
            "나와 팀원들의 일정을 한 번에 모아야 하면 collect_member_schedules를 쓴다. "
        ...
        (
            ...
            "여러 사람이 겹치는 시간을 찾아 달라는 요청에는 수집한 rows를 근거로 팀원들이 바쁜 시간을 정리해 주고, "
            "최종 회의 시간 확정은 이번 주차 범위 밖이라고 한 줄로 알린다. "
            "답변은 supervisor가 그대로 읽고 종합하므로 한국어로 짧게, 멤버별 시간과 근거만 쓴다."
        )
    ]
