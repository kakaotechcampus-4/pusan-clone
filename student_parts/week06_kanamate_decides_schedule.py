...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        "너는 개인 일정 비서 Nana다. 개인 일정 생성/조회/수정/삭제, todo·reminder 저장, 개인 참고자료와 앱 대화 검색(RAG)을 담당한다.",
        "여러 사람의 일정을 맞추는 그룹 조율이나 외부 멤버 일정 조회는 네 담당이 아니다. 그런 요청은 담당이 아니라고 짧게 알린다.",
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        f"너는 그룹 일정 조율 비서 Kana다. 오늘 날짜는 {current_app_date_iso()}이다.",
        "외부 팀원의 과거 대화·일정 조회, 공유 일정 저장소 조회, 여러 사람의 공통 가능 시간 후보 검증과 최종 회의 시간 결정을 담당한다.",
        "그룹 조율 흐름: (1) collect_member_schedules(또는 extract_schedules_from_history)로 내 일정과 외부 멤버 busy-time을 모은다. "
        "(2) 그 busy_rows를 근거로 어떤 일정과도 겹치지 않는 후보를 직접 골라 find_common_available_slots에 candidate_slots로 넘겨 검증한다. "
        "(3) 이어서 decide_final_slot을 호출해 최종 시간을 확정한다. 검증만 하고 끝내지 않는다.",
        "확정된 일정을 앱 DB에 저장하는 것은 Nana 담당이므로 저장은 하지 않는다.",
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            "반드시 nana_agent 또는 kana_agent 중 하나를 호출하고, 그 tool 결과의 answer/payload만 근거로 답한다. 스스로 일정을 계산하거나 지어내지 않는다.",
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "여러 멤버의 공통 가능 시간 후보를 '검증'하는 tool이다. 이 tool은 후보를 대신 계산하지 않는다. "
    "너(agent)가 앞선 일정 조회(collect_member_schedules 등)의 busy_rows를 직접 읽고, 어떤 busy row와도 겹치지 않는 후보를 골라 candidate_slots로 넘겨야 한다. 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), duration_minutes, reason을 포함한다. busy_rows는 앞선 tool output에서 그대로 복사해 함께 넘긴다. "
    "이 결과로 답을 끝내지 말고 이어서 decide_final_slot을 호출해 최종 시간을 확정한다."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "공통 가능 후보 중 최종 회의 시간을 '기록'하는 tool이다. 이 tool은 최종 시간을 대신 고르지 않는다. "
    "너(agent)가 candidate_slots 중 하나를 selected_index 또는 selected_slot으로 직접 고르고, "
    "final_slot을 'YYYY-MM-DD HH:MM-HH:MM' 형식으로 채워 넘긴다. 아직 못 골랐으면 final_slot은 null, "
    "needs_agent_selection은 true로 둔다(확정했으면 false). reason에는 선택/보류 이유를 설명하듯 적고, "
    "근거로 candidate_slots, busy_rows, member_names, date_from, date_to도 함께 넘긴다."
)
...
def find_common_available_slots_dict(
    ...
    if busy_rows is None:
        ...
        busy_rows = json.loads(collected).get("rows", [])
