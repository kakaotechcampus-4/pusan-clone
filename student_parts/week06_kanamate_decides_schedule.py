    ...
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            ...
            "거절하는 내용이면, 그 시간이 참석자와 실제로 겹치지 않는다고 supervisor 자신이 이번 대화 기록(사용자에게 "
            "이미 보여준 이전 턴의 답변)에서 확인한 적이 있는지 직접 판단한다. kana_agent에게 그 여부를 다시 묻지 않는다. "
            "이전 답변에 그 날짜·시간·참석자 조합이 겹치지 않는다고 확정/조회된 내용이 있으면, "
            "nana_agent에게 그 시간과 참석자 정보를 그대로 담아 저장을 위임한다. "
            "이전 대화 기록에 그런 확인 내용이 없으면, 곧바로 nana_agent에게 저장을 맡기지 않는다. "
            "대신 kana_agent에게 '저장'이라는 단어를 쓰지 않고 순수하게 '[날짜] [시간]에 [참석자]와 공통으로 "
            "가능한지 확인해줘'처럼 가용성 확인만 요청한다. 이 재확인 요청에서 kana_agent가 collect_member_schedules와 "
            "find_common_available_slots를 호출해 실제로 겹치지 않는다고 답하면, 그 결과를 근거로 nana_agent에게 "
            "저장을 위임한다. kana_agent가 재확인 요청에도 여전히 거절하거나 겹친다고 답하면, 나나에게 위임하지 않고 "
            "그 사실을 사용자에게 그대로 전달한다.",
        ]
    )
