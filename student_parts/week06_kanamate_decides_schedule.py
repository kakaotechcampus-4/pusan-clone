...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        "supervisor는 직접 업무를 처리하지 않고 Nana 또는 Kana에게 위임한다. ",
        "Nana는 개인 일정/저장/RAG를, Kana는 외부 멤버 일정/공통 시간 결정을 담당한다. ",
        "'민준이랑 회의 잡고 저장해줘'처럼 Nana와 Kana가 다 필요한 요청은 조율/후보 산출은 Kana, 확정 일정의 저장은 Nana와 같이 역할에 맞게 분담시켜서 처리한다. ",
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        "너는 개인일정/저장/RAG를 담당하는 Nana 에이전트이다. ",
        "그룹 조율 요청을 받으면 '그룹 조율은 Kana 담당'이라고 짧게 답한다. ",
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        "너는 외부 멤버 일정/공통 가능 시간/그룹 조율을 담당하는 Kana 에이전트이다. ",
        "확정된 일정 저장은 Nana의 담당이다. ",
        f"오늘 날짜는 {current_app_date_iso()}이다. '어제', '다음주' 등의 모든 상대적인 날짜 표현은 오늘을 기준으로 계산한다. ",
        "'철수가 예전에 뭐라고 했지?'처럼 다른 사람과의 과거 대화를 찾을 때는 search_previous_conversations를 사용한다. ",
        "search_previous_conversations로 찾은 대화의 전체 내용이 필요하면 그 결과의 conversation_id로 load_conversation_messages를 호출한다. ",
        "특정 멤버의 날짜 범위 일정만 필요하면 extract_schedules_from_history를 사용한다. ",
        "'우리 셋 다 언제 바빠?'처럼 나를 포함한 여러 사람의 일정을 함께 봐야 하면 collect_member_schedules를 사용한다. 이 경우 extract_schedules_from_history를 따로 호출하지 않는다. ",
        "그룹 조율 요청에서 참석자 이름이나 날짜를 원문에서 바로 읽기 어려우면 extract_schedule_request(query=사용자 원문)를 먼저 호출한다. 결과는 structured_request 아래에 들어 있고, structured_request.members가 collect_member_schedules의 member_names에 해당한다. ",
        "structured_request.date는 날짜를 하나만 담을 수 있어 기간을 표현하지 못한다. 사용자가 하루를 말했으면 date_from과 date_to에 같은 날짜를 넣고, 기간을 말했으면 끝나는 날짜는 원문과 오늘 날짜를 근거로 직접 계산해 date_to에 채운다. ",
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            "너는 supervisor로서 직접 업무를 처리하지 않고 업무 유형에 따라 Nana 또는 Kana에게 위임한다. ",
            "위임할 때는 사용자 원문과 날짜 등 필요한 맥락을 그대로 query에 담는다. ",
            "직접 호출할 수 있는 tool은 nana_agent, kana_agent 2개 뿐이다. 이전 주차의 tool은 하위 agent가 갖고 있다. ",
            "반드시 nana_agent 또는 kana_agent를 호출해야한다. 두 담당이 모두 필요하면 순서대로 둘 다 호출하고 호출 결과만을 근거로 대답한다. ",
...
DECIDE_FINAL_SLOT_DESCRIPTION = (
    "그룹 회의의 최종 시간을 확정해 기록하는 tool이다. "
    "find_common_available_slots로 후보를 검증한 뒤 반드시 이어서 호출한다. "
    "이 tool은 최종 시간을 대신 골라주지 않는다. 후보 중 어느 것을 쓸지 직접 정해서 넘겨야 한다. "
    "candidate_slots에는 find_common_available_slots가 돌려준 candidate_slots를 그대로 복사해 넘긴다. "
    "고른 후보는 selected_index로 지정한다. index는 0부터 시작하며 첫 번째 후보가 0이다. "
    "final_slot은 'YYYY-MM-DD HH:MM-HH:MM' 형식이다. selected_index를 넘기면 자동으로 만들어지므로 직접 쓰지 않아도 된다. "
    "reason에는 그 시간을 고른 이유를 사용자에게 그대로 보여줄 한 문장으로 쓴다. "
    "후보가 하나도 없거나 사용자에게 더 확인해야 해서 아직 고르지 못했다면 final_slot을 null로 두고 needs_agent_selection을 true로 넘긴다. 이때 임의의 시간을 지어내지 않는다. "
    "판단 근거를 남기기 위해 member_names, date_from, date_to, duration_minutes, busy_rows도 함께 넘긴다. "
    "반환된 final_slot이 최종 답변의 근거다. 답변에는 이 값과 같은 시간을 쓴다."
)
...
def find_common_available_slots_dict(
    ...
    rows = busy_rows
    if rows is None:
        payload = json.loads(collect_member_schedules.invoke({
    ...
    return find_common_available_slots_payload(
        member_names=["나", *[n for n in normalized_members if n != "나"]],
        date_from=normalized_from,
        date_to=normalized_to,
        busy_rows=rows,
