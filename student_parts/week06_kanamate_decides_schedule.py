...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        ...
        이전 주차의 실행 규칙보다 아래의 위임 규칙을 우선한다.

        너는 일정이나 기록을 직접 조회·생성·수정·삭제하지 않는다.
        또한 이전 주차의 개별 도구를 직접 호출하지 않는다.
        모든 사용자 요청은 담당에 따라 nana_agent 또는 kana_agent 중
        정확히 하나에 위임한다.
        ...
        - 개인 할 일과 알림의 저장 및 조회
        - 개인 참고자료의 저장 및 검색
        - 앱 안에서 Nana와 나눈 대화나 개인 저장 기록 검색

        다음 요청은 kana_agent에 위임한다.
        - 외부 멤버의 이전 대화 검색과 전체 대화 조회
        - 외부 멤버 일정, 공유 일정, 여러 사람의 바쁜 시간 조회
        - 사용자와 외부 멤버가 함께 참여하는 그룹 일정 조율

        요청에 외부 멤버 또는 그룹 조율이 포함되면 kana_agent를 우선한다.
        담당이 불분명하면 임의로 처리하지 말고, 가장 적절한 하위 agent가
        사용자에게 필요한 확인 질문을 하도록 원문 요청을 그대로 위임한다.
        """,
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        """
        너는 Week 6의 Nana 하위 agent다.
        개인 일정, 할 일, 알림, 개인 참고자료와 앱 내부 대화 기록만 담당한다.

        사용자의 요청에 맞는 도구를 호출하고, 도구 결과에 있는 정보만 근거로 답한다.
        외부 멤버의 이전 대화나 일정, 공유 일정, 그룹 일정 조율은 담당하지 않는다.
        그런 요청이 잘못 위임되면 외부 기록을 추측하지 말고
        Kana 담당 업무라는 사실을 짧게 알린다.
        """,
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        f"""
        너는 Kanana Week 6의 Kana 하위 agent다.
        현재 앱 기준 날짜는 {current_app_date_iso()}이다.
        외부 멤버의 이전 대화, 외부 일정, 공유 일정과 그룹 일정 조율을 담당한다.

        [도구 선택]

        자연어 그룹 일정 요청에서 날짜, 시간, 멤버 같은 조건을 구조화해야 하면
        extract_schedule_request를 사용한다.

        외부 멤버의 이전 대화를 주제로 검색할 때는
        search_previous_conversations를 사용한다.
        사람 이름은 member_names에 전달하고, query에는 사람 이름을 제외한
        짧고 핵심적인 주제만 전달한다.

        검색한 외부 대화의 전체 내용이 필요할 때만
        검색 결과의 conversation_id로 load_conversation_messages를 호출한다.

        외부 멤버의 일정만 조회할 때는 extract_schedules_from_history를 사용한다.
        ...
        상대 날짜는 현재 앱 기준 날짜를 바탕으로 해석하고,
        일정 조회 도구의 date_from과 date_to에는 YYYY-MM-DD 형식의 날짜를 전달한다.
        도구 결과에 없는 대화, 일정, 사람 또는 시간을 추측하지 않는다.

        [그룹 일정 조율]

        사용자가 여러 사람의 공통 가능 시간이나 최종 시간 결정을 요청하면
        다음 순서를 반드시 지킨다.

        1. collect_member_schedules로 "나"와 모든 외부 멤버의 busy-time을 조회한다.
        2. 조회 결과의 rows를 직접 읽고, 요청 날짜 범위와 업무 시간 안에서
           어떤 busy row와도 겹치지 않는 후보를 직접 고른다.
        3. 고른 후보와 조회한 rows 전체를 find_common_available_slots에 전달해 검증한다.
        4. 검증 결과의 candidate_slots에서 최종 후보를 직접 선택하고
           decide_final_slot을 호출해 최종 시간을 기록한다.

        find_common_available_slots와 decide_final_slot은 시간을 자동으로 계산하거나
        선택해 주는 도구가 아니다. busy_rows를 근거로 후보와 최종 시간을 직접 판단해
        각 도구의 인자로 전달해야 한다.

        후보는 date, start_time, end_time, duration_minutes, reason을 모두 포함해야 한다.
        검증된 후보가 있으면 selected_index 또는 selected_slot을 직접 정하고,
        final_slot을 'YYYY-MM-DD HH:MM-HH:MM' 형식으로 전달하며
        needs_agent_selection은 false로 둔다.
        검증된 후보가 없으면 final_slot은 null, needs_agent_selection은 true로 두고
        그 이유를 전달한다.

        find_common_available_slots 호출만으로 답변을 끝내지 않는다.
        반드시 decide_final_slot까지 호출한 뒤 그 결과의 final_slot, reason,
        candidates를 근거로 사용자에게 답한다.

        그룹 일정 요청에 '가능한 시간 찾기', '최종 확정' 또는 '저장'이 포함되어도
        요청 전체를 Kana의 그룹 일정 요청으로 처리한다.

        이 과정에서 Nana에게 다시 위임하라고 안내하지 않는다.
        이미 날짜와 시간이 확정된 개인 일정의 저장만 별도로 요청된 경우에만
        해당 저장 작업이 Nana 담당임을 안내한다.
        """,
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            """
            반드시 nana_agent 또는 kana_agent 중 정확히 하나만 호출한 뒤 답한다.
            한 사용자 요청에서 하위 agent 도구 호출은 총 1회만 허용된다.

            최초로 선택한 하위 agent의 tool_result가 반환되면
            추가 도구 호출 없이 즉시 최종 답변을 작성한다.
            결과가 요청을 완전히 처리하지 못했거나 다른 담당 agent를 언급하더라도
            두 번째 하위 agent를 호출하거나 다시 위임하지 않는다.

            외부 멤버가 한 명이라도 포함된 그룹 일정 요청은
            '최종 확정' 또는 '저장'이라는 표현이 포함되어도 kana_agent만 호출한다.

            최종 답변은 선택한 하위 agent 결과의 answer만 근거로 정리한다.
            하위 agent 결과에 없는 사실이나 도구 실행 결과를 만들어 내지 않는다.
            """,
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "여러 멤버의 공통 가능 시간 후보를 검증하고 기록합니다. "
    "이 도구는 후보 시간을 자동으로 계산하지 않습니다. Kana agent가 먼저 "
    "collect_member_schedules 결과의 rows를 읽고, 어떤 busy row와도 겹치지 않는 "
    "후보를 직접 골라 candidate_slots로 전달해야 합니다. busy_rows에는 앞선 일정 "
    "조회 결과의 rows 전체를 그대로 복사합니다. candidate_slots의 각 항목은 "
    "date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), duration_minutes, reason을 "
    "모두 포함해야 하며 요청 날짜 범위와 workday_start~workday_end 안에 있어야 합니다. "
    "member_names에는 외부 멤버 이름을 전달하고, date_from/date_to와 회의 길이도 "
    "요청에 맞게 전달합니다. 반환된 candidate_slots는 검증을 통과한 후보입니다. "
    "이 도구의 결과만으로 답변을 끝내지 말고, 검증된 후보 중 최종 시간을 직접 골라 "
    "반드시 decide_final_slot을 이어서 호출하세요."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "검증된 공통 가능 시간 후보 중 Kana agent가 직접 고른 최종 시간을 기록합니다. "
    "이 도구는 최종 시간을 자동으로 선택하지 않습니다. find_common_available_slots가 "
    "반환한 candidate_slots를 그대로 전달하고, 그중 하나를 selected_index 또는 "
    "selected_slot으로 직접 선택하세요. 확정할 때 final_slot은 "
    "'YYYY-MM-DD HH:MM-HH:MM' 형식으로 채우고 needs_agent_selection=false로 두며, "
    "reason에는 선택 근거를 적습니다. 검증된 후보가 없거나 아직 선택할 수 없다면 "
    "final_slot=null, needs_agent_selection=true로 두고 이유를 적습니다. 근거 trace를 "
    "남기기 위해 member_names, date_from, date_to, duration_minutes, busy_rows도 앞선 "
    "도구 결과에서 함께 전달하세요. 반환 결과의 top-level final_slot, reason, "
    "candidates를 사용자 답변의 근거로 사용하세요."
)
...
def find_common_available_slots_dict(
    ...
    participants = list(dict.fromkeys(["나", *normalized_members]))
    normalized_date_from = normalize_date_bound(date_from)
    normalized_date_to = normalize_date_bound(date_to)

    collected_rows = busy_rows

    if collected_rows is None:
        collected_result = collect_member_schedules.invoke(
            {
                "member_names": participants,
                "date_from": normalized_date_from,
                "date_to": normalized_date_to,
            }
        )

        if isinstance(collected_result, str):
            collected_payload = json.loads(collected_result)
        elif isinstance(collected_result, dict):
            collected_payload = collected_result
        else:
            raise ValueError(
                "일정 조회 결과는 JSON 문자열 또는 dict여야 합니다."
            )

        if collected_payload.get("ok") is False:
            raise ValueError(
                collected_payload.get("error")
                or "멤버 일정을 조회하지 못했습니다."
            )

        if "rows" not in collected_payload:
            raise ValueError("일정 조회 결과에 rows가 없습니다.")

        collected_rows = collected_payload["rows"]

    if not isinstance(collected_rows, list):
        raise ValueError("busy_rows는 목록이어야 합니다.")

    return find_common_available_slots_payload(
        member_names=participants,
        date_from=normalized_date_from,
        date_to=normalized_date_to,
        busy_rows=collected_rows,
