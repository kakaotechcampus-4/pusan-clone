    ...
    return [
        *week05_prompt_parts(),
        ...
        (
            ...
            "날짜·시간·사람 이름·제목을 모두 풀어 쓴 자기완결 문장으로 바꿔서 넘긴다. "
            "'다음 주', '내일' 같은 상대 날짜도 오늘 날짜 기준으로 YYYY-MM-DD로 바꿔 적는다. "
            "예: 사용자가 '그럼 그 시간으로 저장해줘'라고 하면 "
            "nana_agent(query='2026-08-12 14:00-15:00에 철수, 영희와 하는 팀 회의 일정을 저장해줘')처럼 넘긴다. "
            "반대로 사용자가 이번 요청에서 말하지 않은 조건(직전 turn의 날짜·id 등)은 query에 끼워 넣지 않는다."
        ),
        (
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        ...
        (
            ...
            "4) 검증 결과에서 하나를 골라 decide_final_slot으로 최종 시간을 확정한다. "
        ...
        (
            ...
            "시간이 '미정'이면 그 부분은 생략한다. 후보 시간대는 '- MM/DD HH:MM ~ HH:MM — 근거' 한 줄로 적는다. "
            "최종 시간을 확정했으면 확정한 시간과 그 이유를 먼저 한 줄로 말하고 근거 일정을 그 뒤에 적는다. "
        ...
        [
            *week06_prompt_parts(),
            (
                ...
                "kana_agent가 final_slot을 확정해 주면 그 시간을 사용자에게 알리고, 사용자가 저장까지 원하면 "
                "그 시간을 풀어 쓴 query로 nana_agent에 저장을 맡긴다."
            ),
        ]
    )
...
DECIDE_FINAL_SLOT_DESCRIPTION = (
    "회의 최종 시간을 기록합니다. "
    "이 tool은 최종 시간을 대신 골라 주지 않습니다. find_common_available_slots가 검증한 후보 중에서 "
    "당신이 직접 하나를 고르고, selected_index(0부터 시작하는 candidate_slots 번호) 또는 selected_slot과 "
    "final_slot을 함께 넘겨야 확정됩니다. "
    "final_slot은 'YYYY-MM-DD HH:MM-HH:MM' 형식의 문자열입니다. 예: final_slot='2026-08-12 14:00-15:00'. "
    "확정했으면 needs_agent_selection=false로 두고, reason에는 왜 그 시간을 골랐는지 사용자에게 그대로 보여줄 수 있는 "
    "설명을 적습니다. "
    "아직 고르지 못했거나 후보가 없으면 final_slot을 null로, needs_agent_selection을 true로 두고 "
    "reason에 확정하지 못한 이유를 적습니다. 이때 임의의 시간을 지어내 채우지 않습니다. "
