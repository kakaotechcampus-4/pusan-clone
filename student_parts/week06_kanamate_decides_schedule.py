...
WEEK06_NANA_PROMPT = """너는 Nana다. supervisor의 system prompt는 전달받지 않으므로 이 지시가 너의 유일한 추가 안내다.
개인 일정 조회·생성·수정·삭제, todo/reminder 저장, 개인 참고자료와 과거 대화 검색(RAG)까지만 담당한다.
외부 멤버 일정 조회나 여러 사람의 공통 가능 시간 조율처럼 나 이외의 사람이 관련된 요청을 받으면
네 담당이 아니라고 짧게 안내하고 직접 처리하지 않는다."""
...
WEEK06_KANA_PROMPT = f"""너는 Kana다. 여러 사람이 관련된 일정 조율을 전담하는 하위 에이전트다.
supervisor나 다른 주차의 system prompt는 전달받지 않으므로 이 지시가 너의 유일한 안내다.
오늘 날짜는 {current_app_date_iso()}이다. "다음 주", "이번 주" 같은 상대 날짜는 이 날짜를 기준으로 계산한다.
자연어로 들어온 일정 요청에서 참여자와 날짜/시간 조건을 구조화할 때는 extract_schedule_request를 사용한다.
...
다시 읽어 다른 시간으로 재시도한다.
확정된 일정을 개인 일정으로 저장하는 것은 네 담당이 아니다. 저장이 필요하면 Nana에게 요청하라고
사용자에게 안내하고 직접 저장 tool을 호출하지 않는다.
find_common_available_slots를 제외한 조회 tool의 결과가 비어 있으면 기록이 없다고 말하고 대화나 일정을 추측하지 않는다."""
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "이 tool은 공통 가능 시간 후보를 스스로 계산하지 않습니다. busy_rows에 담긴 각 멤버의 바쁜 시간을 "
    "네가 직접 읽고, 아무와도 겹치지 않는 시간을 candidate_slots로 골라 넘겨야 검증/기록됩니다. "
    "busy_rows는 새로 만들지 말고 collect_member_schedules 등 앞선 tool output의 rows를 그대로 복사해서 넘기세요. "
    "candidate_slots의 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), "
    "duration_minutes, reason(선택 이유)을 포함해야 하며, busy_rows의 어떤 row와도 시간이 겹치면 안 됩니다. "
    "workday_start~workday_end(기본 09:00~18:00) 범위 밖이거나 duration_minutes보다 짧은 후보, "
    "date_from~date_to 범위 밖 날짜의 후보는 결과에서 조용히 제외됩니다. "
    "candidate_slots 결과가 비어 있다면 공통 시간이 없다는 뜻이 아니라 네가 넘긴 후보가 이 조건 중 하나에 걸려 "
    "제외됐다는 뜻이므로, 사용자에게 실패로 안내하지 말고 busy_rows를 다시 확인해 다른 시간으로 재시도하세요. "
    "이 tool 결과만으로 답변을 끝내지 말고, 검증된 candidate_slots를 그대로 decide_final_slot에 넘겨 "
    "최종 시간을 이어서 결정하세요."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "이 tool은 최종 시간을 스스로 선택하지 않습니다. find_common_available_slots가 검증한 candidate_slots 중 "
    "네가 직접 고른 항목을 selected_index 또는 selected_slot으로, 그리고 확정한 시간을 final_slot으로 "
    "직접 채워 넘겨야 기록됩니다. final_slot 형식은 'YYYY-MM-DD HH:MM-HH:MM'입니다. "
    "아직 확정할 수 없다면 final_slot은 null로 두고 needs_agent_selection을 true로, 확정했다면 false로 넘기세요. "
    "reason에는 선택 또는 보류 이유를 사용자에게 설명할 수 있게 적으세요. "
    "근거를 남기기 위해 candidate_slots, busy_rows, member_names, date_from/date_to도 앞선 tool output에서 "
    "그대로 복사해 함께 넘기세요."
)
...
def find_common_available_slots_dict(
    ...
    if PERSONAL_SHARED_MEMBER_NAME not in normalized_members:
        normalized_members = [*normalized_members, PERSONAL_SHARED_MEMBER_NAME]
    ...
    normalized_busy_rows = [
        {**row, "date": normalize_date_bound(row["date"])} if isinstance(row, dict) and row.get("date") else row
