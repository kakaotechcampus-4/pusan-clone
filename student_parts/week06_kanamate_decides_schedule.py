...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        ...
        "이전 주차에서 최종 회의 시간 확정은 범위가 아니라고 안내한 지시는 이번 주차에 해제된다. "
        "kana_agent가 final_slot을 돌려주면 그 확정 시간을 최종 답변에 반드시 포함한다.",
    ]
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    ...
    "이 tool은 후보를 대신 계산해주지 않는다. collect_member_schedules가 돌려준 busy_rows를 네가 직접 읽고, "
    "어떤 busy row와도 겹치지 않는 시간을 골라 candidate_slots에 채워 넘겨야 한다. "
    "candidate_slots를 비운 채로 호출하면 검증할 후보가 없어 아무 결과도 나오지 않는다. "
    "busy_rows가 비어 있다면 아무도 바쁘지 않다는 뜻이므로 업무 시간 안에서 후보를 직접 만들어 넘긴다. "
    "candidate_slots의 각 항목은 date('YYYY-MM-DD'), start_time('HH:MM'), end_time('HH:MM'), "
    "duration_minutes(정수 분), reason(그 시간을 고른 짧은 근거)을 포함한다. "
    "busy_rows는 앞선 tool output에서 그대로 복사해 함께 넘겨야 검증 근거로 남는다. "
    "date_from~date_to 범위를 벗어나거나, workday_start 이전에 시작하거나 workday_end 이후에 끝나거나, "
    "duration_minutes보다 짧거나, busy_rows와 겹치는 후보는 검증에서 제외된다. "
    "반환값의 후보 목록이 검증을 통과한 결과이고, 비어 있으면 다른 시간대로 후보를 다시 골라 재호출한다. "
    "이 결과만으로 답변을 끝내지 말고 이어서 decide_final_slot을 호출해 최종 시간을 확정한다."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "find_common_available_slots로 검증한 후보 중에서 확정한 최종 회의 시간을 기록하는 tool이다. "
    "이 tool은 최종 시간을 대신 골라주지 않는다. 후보 중 하나를 네가 직접 고른 뒤 "
    "selected_index(또는 selected_slot)와 final_slot을 함께 넘겨야 한다. "
    "final_slot은 'YYYY-MM-DD HH:MM-HH:MM' 형식으로 적는다. "
    "아직 고르지 못했다면 final_slot은 null로 두고 needs_agent_selection을 true로 남긴다. "
    "시간을 확정했다면 needs_agent_selection은 false로 넘긴다. "
    "reason에는 그 시간을 고른 이유나 보류한 이유를 사용자가 읽을 문장으로 적는다. "
...
def find_common_available_slots_dict(
    ...
    # 앞선 tool에서 busy_rows를 복사해 오지 않았다면 여기서 직접 모은다.
    rows = busy_rows
    if rows is None:
...
def decide_final_slot(
    ...
    # 최종 시간은 Kana agent가 고른다. 여기서는 넘어온 값을 그대로 기록만 한다.
...
def kana_agent(query: str) -> str:
    ...
    # 하위 trace에 남은 최종 시간 결정 결과를 supervisor가 쓸 수 있게 끌어올린다.
    final_slot_payload: dict[str, Any] | None = None
    final_decision_payload: dict[str, Any] | None = None
    for event in events:
        content = event.get("content")
