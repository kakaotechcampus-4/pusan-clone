...
SQLITE_MEMORY_PROMPT = "일정, 할 일, 알림은 SQLite에 저장되어 새 대화에서도 유지됩니다. 과거 일정을 물으면 반드시 조회 tool을 사용해서 DB를 조회해 대답합니다."

# TODO: 자연어 구조화 → SQLite 저장과 조회/수정/삭제 tool 호출 순서를 안내하는 규칙을 작성하세요.
WEEK03_TOOL_CALL_PROMPT = "구조화는 StructuredRequest 형식에 맞춰서 구조화합니다. 저장 요청은 extract_schedule_request로 구조화한 뒤, 그 필드를 그대로 save_structured_request에 넘깁니다. 삭제는 personal_list_saved_schedules로 후보를 확인한 뒤 실행합니다. 일정 조회 요청은 personal_list_saved_schedules로, 수정은 후보 확인 후 personal_update_saved_schedule로 처리합니다."
...
class SaveStructuredRequestInput(StructuredRequest):
    ...
    def unwrap_legacy_payload(cls, value: Any) -> Any:
        ...
        # StructuredRequest 인스턴스로 들어오면 dict로 펼쳐 필드 검증에 넘긴다.
        if isinstance(value, StructuredRequest):
            return value.model_dump()
        # 예전 trace/테스트가 한 겹 감싼 wrapper라면 안쪽 dict만 꺼낸다.
    ...
    *,
    ...
    if not delete_all and not has_filter:
        return tool_result(
            ...
            filters={},
            deleted=[],
            reason="삭제 조건이 없습니다. schedule_ids나 날짜/제목/시간, 또는 delete_all을 지정하세요.",
        )
    if delete_all:
...
def save_structured_request(
    ...
    dic = {"kind": kind, "title": title, "date": date, "start_time": start_time, "end_time": end_time, "members": members, "priority": priority, "reason": reason, "original_text": original_text, "source_schedule_id": source_schedule_id}
...
def personal_update_saved_schedule(
    ...
    if result is None:
        return json_payload(tool_result(
            ...
            reason="해당 ID의 저장 일정을 찾을 수 없습니다.",
        ))
    return json_payload(tool_result(
        "personal_update_saved_schedule",
