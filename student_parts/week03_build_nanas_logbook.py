...
SQLITE_MEMORY_PROMPT = (
    "Week 3부터 일정/할 일/알림은 현재 대화의 임시 메모리가 아니라 앱 SQLite DB에 저장된다. "
    "DB에 저장된 기록은 새 대화를 열거나 앱을 다시 시작해도 유지되므로, 저장된 일정을 묻는 질문은 "
    "personal_list_saved_schedules로, 구조화 요청 원본 기록은 list_saved_requests/get_saved_request로 조회한다."
)
    ...
    def unwrap_legacy_payload(cls, value: Any) -> Any:
        ...
        for key in ("payload", "structured_request"):
            ...
            if isinstance(inner, dict):
                source_schedule_id = unwrapped.get("source_schedule_id")
                unwrapped = dict(inner)
                if source_schedule_id and not unwrapped.get("source_schedule_id"):
                    unwrapped["source_schedule_id"] = source_schedule_id
        return unwrapped
...
def _save_input_from(value: SaveStructuredRequestInput | StructuredRequest | dict[str, Any] | str) -> SaveStructuredRequestInput:
    ...
    if isinstance(value, str):
        ...
        # JSON이 아닌 자연어는 Week 2 bridge로 먼저 구조화합니다.
...
def _delete_saved_schedules(
    *,
    ...
    has_explicit_filter = bool(schedule_ids) or any([date, title, start_time, time_unspecified])
    if not delete_all and not has_explicit_filter:
        return tool_result(
            "personal_delete_saved_schedules",
            ok=False,
            error="삭제 조건이 없습니다. schedule_ids나 날짜/제목/시간 필터를 지정하거나 delete_all=True를 사용하세요.",
            deleted_count=0,
            filters=filters,
            deleted=[],
        )
...
def structured_request_from_week01_schedule(schedule: dict[str, Any]) -> SaveStructuredRequestInput:
    ...
    return SaveStructuredRequestInput(
        ...
        reason="Week 1 personal_create_schedule 결과를 SQLite에 이중 기록",
        original_text=json.dumps(schedule, ensure_ascii=False),
        source_schedule_id=str(schedule.get("id") or "") or None,
    )
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        "너는 Week 3 기록장 agent다. Week 2가 구조화까지만 했다면 Week 3는 구조화 결과를 "
        "SQLite에 저장하고 다시 조회/수정/삭제한다. Week 2의 structured_response 계약 대신 "
