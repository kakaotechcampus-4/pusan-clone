...
# 새 대화에서도 SQLite 일정/할 일/알림을 조회할 수 있도록 Week 3 영속 메모리 규칙을 정의합니다.
SQLITE_MEMORY_PROMPT = """Week 3부터 일정, 할 일, 알림의 영속 기록은 SQLite를 기준으로 한다.
save_structured_request로 저장한 내용은 새 대화에서도 유지된다.
저장된 요청은 list_saved_requests/get_saved_request로, 일정은 personal_list_saved_schedules로 조회한다."""
...
def _delete_saved_schedules(
    *,
    ...
    if not delete_all and schedule_ids is None and not any([date, title, start_time, time_unspecified]):
        return tool_result("personal_delete_saved_schedules", ok=False, error="삭제 조건이 필요합니다.", filters=filters, deleted_count=0, deleted=[])
...
def list_saved_requests(
    ...
    # kind/date_from/date_to 필터를 store에 그대로 전달합니다.
...
def get_saved_request(request_id: str) -> str:
    ...
    # 결과가 없을 때도 row=None 응답 형태를 유지합니다.
...
def personal_list_saved_schedules(
    ...
    """앱 DB에 저장된 일정 목록을 날짜/종류/제목 키워드 필터로 반환합니다. Nana가 조회/수정/삭제 후보를 볼 때 사용합니다."""

    # 종류를 생략하면 개인 일정을 기본으로 조회합니다.
    effective_kind = kind or "personal_schedule"
    schedules = _store().list_schedules(
        limit=limit, kind=effective_kind, date_from=date_from, date_to=date_to, keyword=keyword
    )
    filters = {"limit": limit, "kind": effective_kind, "date_from": date_from, "date_to": date_to, "keyword": keyword}
...
def delete_saved_schedules_dict(
    ...
    # 전달받은 store가 있으면 테스트용 store를 우선 사용합니다.
    return _delete_saved_schedules(store=app_store or _store(), schedule_ids=schedule_ids, date=date, title=title,
...
def personal_update_saved_schedule(
    ...
    # None 필드는 기존 값을 유지하고, 존재하지 않는 ID는 실패 응답으로 반환합니다.
    result = _store().update_schedule(schedule_id=schedule_id, title=title, date=date, start_time=start_time,
        end_time=end_time, attendees=attendees)
    if result is None:
        return json_payload(tool_result("personal_update_saved_schedule", ok=False, error="일정을 찾을 수 없습니다."))
...
def personal_delete_saved_schedules(
    ...
    # 공통 삭제 guard를 거쳐 안전하게 삭제합니다.
    result = _delete_saved_schedules(store=_store(), schedule_ids=schedule_ids, date=date, title=title,
        start_time=start_time, time_unspecified=time_unspecified, delete_all=delete_all)
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        # Week 2 구조화 결과를 Week 3 SQLite 저장 흐름으로 연결합니다.
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        f"오늘 날짜는 {current_app_date_iso()}이다. Week 3에서는 SQLite 저장·조회·수정·삭제까지만 수행하고 RAG나 그룹 일정 조율은 하지 않는다.",
    ]
