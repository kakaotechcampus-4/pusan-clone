...
WEEK03_TOOL_CALL_PROMPT = (
    "일정 저장 요청을 받으면 먼저 extract_schedule_request를 호출해 자연어를 구조화한 뒤, "
    "그 결과를 save_structured_request tool에 그대로 전달해 SQLite에 저장한다."
    "저장된 일정은 personal_list_saved_schedules tool로 조회할 수 있고, "
    "저장된 요청 원본을 볼 때는 list_saved_requests 또는 get_saved_request tool을 호출한다."
)
...
def _save_input_from(value: SaveStructuredRequestInput | StructuredRequest | dict[str, Any] | str) -> SaveStructuredRequestInput:
    ...
    raise RuntimeError(f"저장 입력으로 변환할 수 없는 형태: {type(value)}")
...
def save_structured_request_payload(
    ...
    *,
    ...
    result = active_store.save_structured_request(save_input.model_dump())
    ...
    *,
    ...
    if not delete_all and not has_filter:
        return tool_result(
            ...
            filters=filters, 
            deleted=[], 
            error="No deletion criteria provided. Specify schedule_ids or filters, or set delete_all=True.",
        )
    if delete_all:
...
def structured_request_from_week01_schedule(schedule: dict[str, Any]) -> SaveStructuredRequestInput:
    ...
    return SaveStructuredRequestInput(
        ...
        original_text=schedule.get("original_text") or "",
    )
...
def personal_update_saved_schedule(
    ...
    if result is None:
        return json_payload(tool_result("personal_update_saved_schedule", ok=False, schedule_id=schedule_id, error="Schedule ID not found",))
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        "Week 2에서는 구조화까지만 했지만, Week 3부터는 구조화한 StructuredRequest를 반드시 SQLite에 저장하고, 저장된 요청/일정을 다시 조회/수정/삭제할 수 있다.",
        "앞선 주차의 '저장하지 않는다'는 지시보다 이 규칙을 우선시해야된다.",
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
