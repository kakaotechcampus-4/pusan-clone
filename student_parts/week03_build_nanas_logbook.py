...
Week 3부터 일정, 할 일, 알림은 앱 SQLite DB에 영속적으로 저장한다.
현재 대화의 임시 메모리만 신뢰하지 말고, 새 대화에서도 저장된 데이터를 SQLite 조회 도구로 확인한다.
저장·조회·수정·삭제 결과는 도구가 반환한 JSON을 기준으로 답하고, 존재하지 않는 ID나 데이터를 추측하지 않는다.
"""

WEEK03_TOOL_CALL_PROMPT = """
자연어 저장 요청은 extract_schedule_request로 구조화한 뒤 structured_request 필드를 save_structured_request에 전달한다.
저장 기록은 list_saved_requests 또는 get_saved_request로 조회한다.
일정 수정·삭제 전에는 personal_list_saved_schedules로 대상과 schedule_id를 확인한다.
수정 시 실제 변경 필드만 전달하고, 삭제 시 schedule_ids 또는 명확한 필터를 전달한다. 조건 없는 삭제는 실행하지 않는다.
"""
...
class SaveStructuredRequestInput(StructuredRequest):
    ...
    def unwrap_legacy_payload(cls, value: Any) -> Any:
        ...
        # Pydantic 모델은 저장 스키마가 검증할 수 있도록 dict로 변환합니다.
        normalized = value.model_dump() if isinstance(value, StructuredRequest) else value

        # 예전 trace에서 사용한 wrapper가 여러 겹이어도 실제 저장 payload까지 순서대로 풉니다.
        while isinstance(normalized, dict):
            nested_value: Any | None = None
            for wrapper_key in ("structured_request", "payload"):
                candidate = normalized.get(wrapper_key)
                if isinstance(candidate, StructuredRequest):
                    nested_value = candidate.model_dump()
                    break
...
def _save_input_from(value: SaveStructuredRequestInput | StructuredRequest | dict[str, Any] | str) -> SaveStructuredRequestInput:
    ...
    normalized: StructuredRequest | dict[str, Any]
    if isinstance(value, str):
        text = value.strip()
        try:
            parsed_json = json.loads(text)
        except json.JSONDecodeError:
            # JSON이 아니면 Week 2 helper로 자연어를 먼저 구조화합니다.
            normalized = extract_structured_request(text)
        else:
            if not isinstance(parsed_json, dict):
                raise ValueError("저장할 JSON은 객체(dict) 형태여야 합니다.")
...
def save_structured_request_payload(
    ...
    *,
    ...
    storage_payload = validated_request.model_dump(exclude_none=True)
    saved_result = (store or _store()).save_structured_request(storage_payload)
    return tool_result("save_structured_request", **saved_result)
...
def _delete_saved_schedules(
    *,
    ...
    has_explicit_filter = schedule_ids is not None or any((date, title, start_time, time_unspecified))

    # 전체 삭제는 delete_all=True일 때만 허용하고, 일반 삭제는 명시 조건이 있어야 합니다.
    if not delete_all and not has_explicit_filter:
        return tool_result(
...
def personal_create_schedule(
    ...
    # 기존 Week 1 생성 도구를 재사용해 임시 일정을 만든 뒤 같은 일정을 SQLite에도 저장합니다.
...
def save_structured_request(
    ...
    # args_schema 검증을 통과한 값 중 실제 값이 있는 필드만 저장소에 전달합니다.
    storage_payload = {
        key: value
        for key, value in {
            "kind": kind,
    ...
    saved_result = _store().save_structured_request(storage_payload)
    return json_payload(tool_result("save_structured_request", **saved_result))
...
def personal_update_saved_schedule(
    ...
    # None은 "수정하지 않음"이므로 실제 값이 전달된 필드만 업데이트합니다.
    updates = {
        key: value
        for key, value in {
            "title": title,
    ...
    update_result = _store().update_schedule(schedule_id=schedule_id, **updates)
    ...
    if update_result is None:
        return json_payload(
            tool_result(
                ...
                shared_sync=None,
                error=f"schedule_id={schedule_id!r}인 일정을 찾을 수 없습니다.",
            )
        )
    ...
    return json_payload(
        tool_result(
            ...
            shared_sync=update_result.get("shared_sync"),
        )
    )
...
Week 3에서는 자연어 구조화와 SQLite 기반 저장·조회·수정·삭제까지만 수행한다.
저장은 extract_schedule_request 다음 save_structured_request를 사용한다.
수정·삭제 전에는 personal_list_saved_schedules로 대상과 schedule_id를 확인한다.
Week 4 이후의 RAG 검색이나 외부 멤버 일정 조율 기능은 수행하지 않는다.
""",
    ]
...
def build_week03_agent() -> object:
    ...
    if _WEEK03_AGENT is None:
        # Agent는 한 번만 생성하고 이후 호출에서 재사용합니다.
