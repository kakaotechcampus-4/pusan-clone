...
SQLITE_MEMORY_PROMPT = (
    "Week 3부터 일정·할 일·알림은 대화 임시 메모리가 아니라 앱 SQLite DB에 영구 저장된다. "
    "저장한 기록은 앱을 다시 시작하거나 새 대화를 열어도 그대로 남아 있으므로, "
    "조회 요청에는 반드시 저장 tool 결과를 근거로 답한다."
)

# TODO: 자연어 구조화 → SQLite 저장과 조회/수정/삭제 tool 호출 순서를 안내하는 규칙을 작성하세요.
WEEK03_TOOL_CALL_PROMPT = (
    "저장 요청은 먼저 extract_schedule_request로 자연어를 구조화한 뒤, 그 structured_request의 "
    "kind/title/date/start_time/end_time/members/priority/reason/original_text를 그대로 "
    "save_structured_request 인자로 넘겨 한 번만 저장한다. "
    "일정 조회 요청은 personal_list_saved_schedules를, 저장된 요청 원본 확인은 "
    "list_saved_requests 또는 get_saved_request를 사용한다."
)
        ...
        if isinstance(value, dict):
            for wrapper_key in ("structured_request", "payload"):
                ...
                if isinstance(inner, dict):
                    merged = {key: item for key, item in value.items() if key not in {"structured_request", "payload"}}
                    merged.update(inner)
                    return merged
        return value
...
def save_structured_request_payload(
    ...
    *,
    ...
    saved_input = _save_input_from(request)
    payload = {key: item for key, item in saved_input.model_dump().items() if item is not None}
...
def _delete_saved_schedules(
    *,
    ...
    if not delete_all and not schedule_ids and not any([date, title, start_time, time_unspecified]):
        return tool_result(
            "personal_delete_saved_schedules",
            ok=False,
            error="삭제 조건이 없습니다. schedule_ids나 date/title/start_time 필터를 주거나 delete_all=True로 요청하세요.",
            deleted_count=0,
            filters=filters,
            deleted=[],
        )
    ...
    if updated is None:
        return json_payload(
            tool_result(
                ...
                error="해당 schedule_id의 저장 일정을 찾지 못했습니다.",
            )
        )
    return json_payload(
        tool_result(
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        "Week 2에서 만든 StructuredRequest는 최종 답변으로 끝내지 말고 Week 3 저장 tool까지 이어서 SQLite에 남긴다.",
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        # TODO: 현재 날짜, Week 3 tool 선택 기준, 이번 주차의 범위를 설명하는 agent 지시를 추가하세요.
        (
            f"너는 개인 일정 메이트 나나(Nana)다. 오늘은 {current_app_date_iso()}이다. "
            "저장·조회가 필요하면 알맞은 Week 3 SQLite tool을 호출한 뒤 결과를 바탕으로 한국어로 짧게 답한다. "
            "이번 주차는 구조화 결과의 저장과 조회까지만 다루고, RAG나 외부 멤버 일정 조율은 하지 않는다."
        ),
    ]
