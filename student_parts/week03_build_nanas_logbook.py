...
제목이나 날짜 같은 필터만으로 삭제를 요청하면 후보가 2건 이상일 때 코드가 삭제를 막고
ok=False와 candidates를 반환하므로, candidates에서 schedule_id를 골라 schedule_ids로 다시 요청한다.
후보가 여러 개면 임의로 고르거나 한꺼번에 변경하지 말고 사용자가 대상을 선택할 수 있게 설명한다.
같은 제목의 여러 일정을 한꺼번에 지우려면 제목 필터에 의존하지 말고
사용자가 확인한 schedule_ids 목록이나 명시적인 delete_all=True를 사용한다.
...
#      - _delete_saved_schedules(...)가 삭제 정책을 코드로 강제합니다. 조건이 없으면 막고,
#        확인된 schedule_ids나 delete_all은 그대로 처리하며, 제목·날짜·시간 필터만으로는
#        후보가 2건 이상일 때 삭제하지 않고 candidates를 돌려줍니다(1건이면 그 일정만 삭제).
#      - deleted_count, filters, deleted를 유지하고, 후보 반환 시 candidates도 함께 전달해
#        trace에서 무엇이 지워졌는지 또는 왜 막혔는지 확인할 수 있게 합니다.
#
...
#     삭제 정책을 코드로 강제하는 안전 경계입니다. 조건이 없으면 막고, 확인된 schedule_ids와
#     delete_all은 바로 삭제하며, 필터 삭제는 find_schedules로 후보를 먼저 조회해
#     1건이면 삭제·2건 이상이면 candidates만 반환합니다. 실제 SQL 삭제는 AppSQLiteStore가 수행하고,
#     이 함수는 정책 판단과 응답 모양(deleted/candidates)을 정리합니다.
#
...
#     schedule_ids나 날짜/제목/시간 필터로 저장 일정을 삭제하는 tool입니다. 조건 없는 삭제와
#     필터에 여러 건이 매칭되는 삭제를 실패 응답으로 막고, 후자는 candidates를 반환합니다.
#
#   - [공통] week03_tools()
...
def _delete_saved_schedules(
    *,
    ...
    # 사용자가 확인한 schedule_id는 일정을 하나씩 정확히 가리키므로,
    # 후보 조회 없이 그대로 삭제한다. 여러 ID를 명시한 일괄 삭제도 사용자가 확인한 경로다.
    if schedule_ids:
    ...
    # 제목·날짜·시간 필터만으로는 같은 이름의 여러 일정이 한꺼번에 지워질 수 있으므로,
    # 실제 삭제 전에 후보를 먼저 조회한다. 100건 초과 여부까지 알 수 있게 상한을 101로 넘긴다.
    ...
    # 후보가 2건 이상이면(101건 이상 포함) 프롬프트가 아니라 코드에서 삭제를 막고,
    # 사용자가 candidates에서 schedule_id를 확인해 다시 요청하도록 후보만 돌려준다.
    if len(candidates) > 1:
        # 상한(101)까지 조회했으므로 100건이 넘으면 후보 목록이 잘렸음을 명시한다.
        # 삭제를 막는다는 결정은 동일하지만, 잘린 목록을 전부라고 오해하지 않도록 신호를 남긴다.
        truncated = len(candidates) > 100
        return tool_result(
            "personal_delete_saved_schedules",
            ok=False,
            error=(
                "여러 일정이 매칭되어 삭제하지 않았습니다. "
                "candidates에서 schedule_id를 확인해 다시 요청하세요."
                + (" 매칭이 100건을 넘어 후보 목록은 일부만 표시했습니다." if truncated else "")
            ),
            filters=filters,
    ...
    # 일치하는 일정이 없는 것은 오류가 아니라 삭제 0건의 정상 결과다.
    # 정확히 한 건이면 그 일정만 schedule_id로 삭제한다.
    if candidates:
        deleted = store.delete_schedules_by_filter(
            schedule_ids=[candidates[0]["schedule_id"]],
        )
    else:
        deleted = []
...
def personal_delete_saved_schedules(
    ...
    schedule_ids나 delete_all은 사용자가 확인한 대상으로 보고 바로 삭제하지만,
    제목·날짜·시간 필터만으로 여러 일정이 매칭되면 삭제하지 않고 ok=False와 candidates를
    돌려줍니다. 그때는 candidates에서 schedule_id를 골라 schedule_ids로 다시 호출합니다.
    """
