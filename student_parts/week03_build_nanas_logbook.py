...
SQLITE_MEMORY_PROMPT = (
    "Week 3부터 일정·할 일·알림은 임시 메모리가 아니라 앱 SQLite DB에 영구 저장된다. "
    "새 대화나 앱 재시작 이후에도 저장된 기록이 유지되므로, '내 일정 보여줘' 같은 조회 요청에는 "
    "저장된 데이터를 tool로 조회해서 답한다."
)

WEEK03_TOOL_CALL_PROMPT = (
    "저장 요청은 먼저 extract_schedule_request로 자연어를 구조화한 뒤, 그 structured_request 필드를 "
    "save_structured_request에 그대로 넘겨 저장한다. "
    "수정·삭제 요청은 먼저 personal_list_saved_schedules로 대상과 schedule_id를 확인한 뒤 "
    "personal_update_saved_schedule 또는 personal_delete_saved_schedules를 호출한다. "
    "삭제는 조건 없이 하지 말고 schedule_id나 날짜·제목·시간 조건을 반드시 명시한다."
)
...
def save_structured_request_payload(
    ...
    *,
    ...
    saved = target.save_structured_request(payload)
    ...
    *,
    ...
    if not delete_all and not has_filter:
        return tool_result(
            ...
            ok=False,
            reason="삭제 조건이 없습니다. schedule_id나 날짜·제목·시간 조건을 지정하거나 delete_all=True로 명시하세요.",
            deleted_count=0,
            filters=filters,
            deleted=[],
        )
    ...
    if result is None:
        return json_payload(
            tool_result(
                ...
                ok=False,
                reason=f"schedule_id '{schedule_id}' 에 해당하는 일정을 찾지 못했습니다.",
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        f"현재 날짜는 {current_app_date_iso()}이며 상대 날짜는 이 날짜를 기준으로 해석한다. "
        "Week 3의 범위는 구조화한 요청을 SQLite에 저장하고 다시 조회·수정·삭제하는 것까지다. "
