...
class SaveStructuredRequestInput(StructuredRequest):
    ...
    def unwrap_legacy_payload(cls, value: Any) -> Any:
        ...
        if isinstance(value, dict):
            # 예전 trace/테스트는 실제 필드를 payload/structured_request 안에 감쌌다. 한 겹만 벗긴다.
...
def save_structured_request_payload(
    ...
    *,
    ...
    saved = target.save_structured_request(save_input.model_dump())
    ...
    *,
    ...
    if not delete_all and not has_filter:
        return tool_result(
            tool_name,
            ok=False,
            error="삭제 조건이 없습니다. schedule_ids/date/title/start_time/time_unspecified 중 하나 이상을 지정하거나 delete_all=True로 명시하세요.",
            deleted_count=0,
            filters={},
            deleted=[],
        )
    ...
    if delete_all:
        ...
        if not deleted:
            return tool_result(
                tool_name,
                ok=False,
                error="조건에 맞는 저장 일정을 찾지 못했습니다.",
                deleted_count=0,
                filters=filters,
                deleted=[],
            )

    return tool_result(tool_name, deleted_count=len(deleted), filters=filters, deleted=deleted)
...
def personal_create_schedule(
    ...
    # Week 1 생성이 실패하면(잘못된 날짜/시간 등) SQLite 저장 없이 그대로 알린다.
