...
def week04_prompt_parts() -> list[str]:
    ...
    return [
        *week03_prompt_parts(),
        ...
        "일정 생성 요청에 start_time이 없으면 사용자가 선호를 명시하지 않았더라도 반드시 search_personal_references로 일정 종류에 맞는 시간 선호를 먼저 검색하여라.",
        "시간 선호 검색 결과가 나오기 전에는 일정 생성 도구를 호출하지 말고, start_time에 '미정'을 넣어 임의로 일정을 생성하지 말아라.",
        "검색 결과에 정확한 시간이 있으면 그 값을 사용하고, 선호 시간 범위가 있으면 그 범위의 시작 시각을 start_time으로 사용하여라.",
        "관련된 시간 선호를 찾지 못했거나 검색 결과끼리 충돌할 때만 일정 생성 전에 사용자에게 시간을 물어보아라.",
        "사용자 표현에서 '회의', '진료', '운동'처럼 일정 종류가 명확하면 그 표현을 제목으로 사용하고 제목을 다시 묻지 말아라.",
        ...
        "사용자: '다음 주 수요일에 내 선호 시간대로 팀 회의를 잡아줘.' → 시간이 명시되지 않았으므로 search_personal_references(query='회의 시간 선호')로 저장된 선호를 먼저 찾아라. 선호 시간이 10:00~12:00이면 10:00을 start_time으로 사용하여 일정을 생성하여라.",
        "사용자: '내일 회의 하나 잡아줘.' → 제목은 '회의', 날짜는 내일로 정하고 search_personal_references(query='회의 시간 선호')를 먼저 사용하여라. 선호 시간이 10:00~12:00이면 start_time을 10:00으로 정해 일정을 생성하고, 관련 선호가 없을 때만 사용자에게 시간을 물어보아라.",
