...
def add_personal_reference_dict(
    ...
    *,
    ...
    if tags is None:
        tags = []
    metadata = reference_store.add_personal_reference(title, content, tags)
    return {
...
def add_personal_reference(title: str, content: str, tags: list[str] | None = None) -> str:
    ...
    res = add_personal_reference_dict(REFERENCE_STORE, title=title, content=content, tags=tags)
...
def search_personal_references(query: str, top_k: int = 2) -> str:
    ...
    limit = safe_limit(top_k, default=2)
...
def search_saved_requests(query: str, top_k: int = 3) -> str:
    ...
    limit = safe_limit(top_k, default=3)
...
def week04_prompt_parts() -> list[str]:
    ...
    return [
        *week03_prompt_parts(),
        ...
        "일정을 날짜 기준으로 조회할 때는 personal_list_saved_schedules를 사용한다. ",
        "할일/리마인더를 날짜 기준으로 조회할 때는 list_saved_requests를 사용한다. ",
