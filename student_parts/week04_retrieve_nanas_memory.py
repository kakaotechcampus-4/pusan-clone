...
def add_personal_reference_dict(
    ...
    *,
    ...
    added = reference_store.add_personal_reference(
        title=title, content=content, tags=tags or []
    )

    return {
        "reference_backend": added.pop("backend", None),
        "reference_row": added,
    }
...
def search_saved_request_rows(
    ...
    *,
    ...
    saved_requests = sqlite_store.search_saved_requests(
        query=query,
        limit=top_k,
    )
    ...
    return [
        {
            ...
            "members": _decode_attendees(request.get("members_json")),
...
def add_personal_reference(
    ...
    added = add_personal_reference_dict(
        reference_store=REFERENCE_STORE, title=title, content=content, tags=tags
    )
    return json_payload(
        {
            "reference_backend": added["reference_backend"],
            "reference": added["reference_row"],
        }
    )
...
def search_saved_requests(query: str, top_k: int = 3) -> str:
    ...
    rows = search_saved_request_rows(
        ...
        query=query,
        top_k=safe_limit(top_k, default=3),
    )
...
def search_nana_memory(
    ...
    reference_hits = search_personal_reference_hits(
        REFERENCE_STORE, query=query, top_k=limit
    )
    ...
    if attendee:
        schedules = [
            ...
            if attendee in schedule.get("attendees", [])
        ]

    schedule_chunks = [
        f"{schedule.get('date') or ''} {schedule.get('start_time') or ''} {schedule['title']}".strip()
        for schedule in schedules[:limit]
    ]

    context = "\n".join(
        [hit["content"] for hit in reference_hits] + schedule_chunks
    )
...
def week04_prompt_parts() -> list[str]:
    ...
    return [
        *week03_prompt_parts(),
        "사용자가 개인 메모나 참고자료를 적어 두거나 기억해 달라고 하면(예: '메모해 둬', '기억해 둬', '참고로 적어 놔') "
        ...
        "질문이 참고자료와 저장 기록 두 출처에 걸치면 search_personal_references와 search_saved_requests를 각각 호출해 근거를 합쳐 답한다. "
        "앞선 지시 중 '조회 tool은 이번 요청에서 최대 1번만 호출한다'는 Week 4 검색 tool에는 적용하지 않는다. "
        "단, 같은 tool을 같은 query로 반복 호출하지 않는다.",
        "지난 대화에서 했던 말이나 논의를 묻는 질문(예: '전에 ~ 얘기했었지?')은 "
        "search_conversation_messages를 호출해 hits의 대화 내용을 근거로 답한다. "
        "현재 진행 중인 대화는 검색에서 자동 제외되므로, 방금 한 말은 검색하지 말고 대화 맥락으로 답한다. "
        "대화 검색 결과에서 assistant 발화만으로 사실을 확정하지 말고 user 발화를 우선 근거로 삼는다.",
        "이 에이전트의 범위는 개인 참고자료 저장·검색, SQLite 저장 기록의 주제어 검색, "
        "지난 대화(채팅 발화) 검색까지 확장된다.",
    ]
