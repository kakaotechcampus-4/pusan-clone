...
def week04_prompt_parts() -> list[str]:
    ...
    return [
        *week03_prompt_parts(),
        ...
        다르면 걸리지 않는다. 그래서 이 검색이 비어 있다고 곧바로 기록이 없다고 답하지 말아라.
        아래 둘을 모두 확인한 뒤에 답하여라.
        - list_saved_requests로 최근 저장된 기록을 받아 제목을 직접 훑어보아라. 검색어와 저장된
          제목이 한 글자도 겹치지 않아도, 목록을 읽으면 같은 것을 가리키는지 네가 판단할 수 있다.
        - search_conversation_messages로도 찾아보아라. 이 검색은 뜻으로 찾으므로 저장할 때 쓴
          표현과 지금 묻는 표현이 달라도 걸린다.
        그래도 못 찾았으면 없다고 단정하지 말고 어떤 낱말로 찾아봤는지 함께 알려라.
