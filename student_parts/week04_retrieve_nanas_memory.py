...
WEEK04_MEMORY_PROMPT = (
    ...
    "- 개인 메모·선호·취향·습관처럼 자연어로 적어 둔 내용: search_personal_references 를 쓴다(ChromaDB 의미 검색, 결과는 hits).\n"
    "  예) '회의를 몇 시에 선호한다고 했지', '점심시간에 대해 내가 적어둔 메모', '내 작업 습관' — 회의·시간이 언급돼도 '선호/취향/메모'를 묻는다면 여기다.\n"
    "- 구체적 날짜·시간으로 등록된 일정/할 일/알림 기록: search_saved_requests 를 쓴다(SQLite 검색, 결과는 rows).\n"
    "  예) '저장된 팀 회의 일정', '등록한 할 일 목록', 'group_schedule/todo/reminder로 저장한 것'.\n"
    "- 앱에 저장된 지난 대화에서 오갔던 말: search_conversation_messages 를 쓴다(대화 RAG, 결과는 hits/rows/context).\n"
    "선호·취향·메모(자연어로 적어둔 것)와 등록된 일정(구조화 기록)을 헷갈리지 않는다: "
    "'선호한다/좋아한다/적어뒀다'는 참고자료(search_personal_references), '언제로 잡혀 있다/등록했다'는 저장 일정(search_saved_requests)이다.\n"
