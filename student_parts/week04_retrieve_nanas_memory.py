...
def search_nana_memory(
    ...
    fetch_limit = limit * 5 if attendee else limit
    schedules = SQLITE_STORE.list_schedules(limit=fetch_limit, date_from=date_from, date_to=date_to)
    if attendee:
        schedules = [row for row in schedules if attendee in (row.get("attendees") or [])][:limit]

    lines = ["[개인 참고자료]"]
    if reference_hits:
        for hit in reference_hits:
            lines.append(f"- {hit['metadata']['title']}: {hit['content']}")
    else:
    ...
    if schedules:
        for row in schedules:
            attendees_text = ", ".join(row.get("attendees") or []) or "참석자 미정"
            time_range = f"{row.get('start_time') or ''}-{row.get('end_time') or ''}"
            lines.append(f"- {row.get('title')} | {row.get('date') or '날짜 미정'} {time_range} | {attendees_text}")
    else:
        lines.append("- 조건에 맞는 일정이 없습니다.")
