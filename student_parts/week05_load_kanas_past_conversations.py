...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    saved_schedules = AppSQLiteStore(CONFIG.app_db_path).list_schedules(limit=100)
    saved_schedule_ids = {
        schedule_id
...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        "반드시 collect_member_schedules를 호출해.",
        "사용자가 특정 날짜나 멤버를 지정하지 않고 공유 일정이나 팀 일정을 조회해달라고 요청하는 경우, "
        "사용자에게 필터를 되묻지 말고 인자 없이 list_shared_schedules를 즉시 호출해.",
        "사용자가 본인('나')의 공유 일정을 물어볼 때는 member_names=['나']를 인자로 넘겨",
        "사용자가 '조회', '확인', '보여줘', '알려줘' 같은 조회/읽기 요청을 할 때는 "
        "이전 대화 맥락과 관계없이 절대로 등록이나 삭제 도구(create_shared_schedule, delete_shared_schedule)를 다시 호출하지 말고 오직 list_shared_schedules 도구만 호출해.",
        "공유 일정(create_shared_schedule)을 등록할 때는 연동 및 동시 삭제가 가능하도록 동일한 source_conversation_id(예: 고유 식별자 또는 회의 그룹 ID)를 지정해.",
        "상대방과 함께하는 회의나 약속인 경우, 상대방(예: 철수)뿐만 아니라 '나'의 공유 일정 저장소에도 추가될 수 있도록 member_name='상대방'과 member_name='나'로 create_shared_schedule을 각각 호출해.",
        "공유 일정을 삭제(delete_shared_schedule)할 때는 해당 회의의 source_conversation_id 또는 schedule_id를 지정하여 연관된 공유 일정이 함께 삭제되도록 해.",
