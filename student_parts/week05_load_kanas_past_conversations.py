...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    # kind 필터를 걸지 않는다: schedules row 는 개인이든 그룹이든 owner 가 '나'인
    # 내 일정이고, 둘 다 그 시간에 내가 바쁘다는 근거다. 필터를 걸면 이미 잡아둔
    # 그룹 회의가 "빈 시간"으로 추천되는 버그가 생긴다.
...
def _collect_member_schedules(
    *,
    ...
    for schedule in personal_schedules:
        ...
        rows.append(
            {
                "member_name": PERSONAL_SHARED_MEMBER_NAME,  # 4. 내 일정을 member_name="나" 형식으로 변환
    ...
    if external_members:
        external_payload = json.loads(
            call_mcp_tool_sync(  # 5. 외부 멤버 일정 MCP 조회
        ...
        for external_row in (
            external_payload.get("rows") or []
        ):  # 6. 같은 rows 배열로 병합
            if isinstance(external_row, dict):
                rows.append(dict(external_row))

    # 같은 일정이 앱 DB 경로와 공유 저장소 경로로 두 번 들어오면 하나로 걸러낸다.
    # 내 일정(위에서 먼저 append)이 앞에 있으므로 그 notes 가 남는다.
    rows = _dedupe_schedule_rows(rows)

    rows.sort(  # 7. 날짜 -> 시작 시간 -> 이름 순 정렬
        key=lambda row: (
            str(row.get("date") or ""),
    ...
    return {
        ...
        "date_to": normalized_date_to,
        "rows": rows,
        "schedule_summary": external_schedule_summary(rows),  # 8. schedule_summary 생성
    }
