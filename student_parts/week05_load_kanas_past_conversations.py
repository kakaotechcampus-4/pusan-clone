...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    """SQLite 저장 일정과 현재 대화의 임시 일정을 group 조율 후보로 사용합니다.

    schedules 테이블의 row는 개인이든 그룹이든 owner가 '나'인 내 일정이므로 kind로 거르지 않습니다.
    """
    ...
    for schedule in PERSONAL_SCHEDULES:
        if _schedule_scope(schedule) != session_id:
            continue
        schedule_key = str(schedule.get("schedule_id") or schedule.get("id") or "")
        if schedule_key and schedule_key in seen_ids:
            continue
        seen_ids.add(schedule_key)
...
def _dedupe_schedule_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """앱 DB와 공유 저장소 양쪽에서 들어온 같은 일정을 앞선 row 하나만 남깁니다."""
...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        "Week 5부터는 내 기록과 다른 사람의 기록을 서로 다른 저장소에서 읽는다. "
        "내 참고자료·저장 일정·앱 대화는 Week 3-4 tool로 다루고, 외부 멤버의 이전 대화와 일정은 외부 SQLite/MCP tool로만 조회한다. "
        "외부 멤버가 예전에 무슨 이야기를 했는지 확인해야 하면 search_previous_conversations로 찾고, "
        "결과 row의 conversation_id를 load_conversation_messages에 넘겨 그 대화의 전체 메시지를 읽는다. "
        "member_names를 넘기지 않으면 모든 멤버를 검색하고 빈 목록을 넘기면 아무것도 검색되지 않으므로, 대상이 불분명하면 넘기지 않는다. "
        "특정 멤버가 언제 바쁜지 날짜 범위로 물으면 extract_schedules_from_history를 호출하고, "
        "member_names에는 사용자가 말한 이름을 그대로, date_from과 date_to는 위 현재 날짜를 기준으로 계산한 YYYY-MM-DD를 넣는다. "
        "공유 일정 저장소에 실제로 등록된 row를 확인해야 하면 list_shared_schedules를 호출한다. 내 일정 복사본은 member_name이 '나'로 등록되어 있다. "
        "나와 다른 사람의 일정을 함께 봐야 하는 요청은 collect_member_schedules 하나로 처리하고, "
        "여러 tool을 따로 호출해 결과를 직접 이어 붙이지 않고 rows와 schedule_summary를 근거로 답한다. "
        "create_shared_schedule과 delete_shared_schedule은 사용자가 공유 일정 등록이나 삭제를 명시적으로 요청할 때만 사용한다. "
        "Week 5의 범위는 여러 사람의 바쁜 시간을 모아 보여주는 데까지다. "
        "최종 회의 시간을 혼자 확정하지 말고 후보를 제시한 뒤 사용자에게 확인을 받는다."
    ]
