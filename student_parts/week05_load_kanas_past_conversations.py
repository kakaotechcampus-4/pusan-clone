...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    stored_ids = {
        ...
        for row in stored
        if row.get("schedule_id") or row.get("id")
    }
    ...
    temp_schedules = [
        ...
        and (schedule.get("schedule_id") or schedule.get("id")) not in stored_ids
    ]

    return [*stored, *temp_schedules]
...
def _dedupe_schedule_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    앱 DB에 저장된 내 일정은 공유 저장소에도 `"나"` 이름으로 자동 동기화되므로,
    member_names에 "나"가 들어온 호출에서는 같은 일정이 두 경로로 들어옵니다.
    앞에 오는 앱 DB row(my_rows)를 남깁니다.

    두 경로가 같은 일정을 서로 다르게 다듬기 때문에 값을 그대로 비교하면 안 됩니다.
    공유 저장소는 제목의 소괄호를 지우고 공백을 하나로 줄이므로 제목은 다듬어서 비교하고,
    end_time은 경로마다 다르게 채워질 수 있어 키에서 뺍니다.
    """
...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        "collect_member_schedules 같은 외부 MCP tool을 반드시 사용해 조회한다. "
        "만약 일정을 찾지 못할 경우 새로운 일정을 생성해서는 안되고 해당 일정이 없다는 것을 사용자에게 알린다. "
        "search_previous_conversations로 관련 대화를 찾은 뒤, 더 자세한 내용이 필요하면 "
