...
class StructuredRequest(BaseModel):
    ...
    title: str | None = Field(default=None, description="일정 또는 할 일 제목. 확실하지 않으면 None")
    date: str | None = Field(default=None, description="YYYY-MM-DD. 확실할 때만 채우고 모르면 None")
    start_time: str | None = Field(default=None, description="HH:MM. 확실할 때만 채우고 모르면 None")
    end_time: str | None = Field(default=None, description="HH:MM. 확실할 때만 채우고 모르면 None")
    ...
    reason: str | None = Field(default=None, description="이렇게 구조화한 판단 근거")
    original_text: str = Field(default="", description="사용자가 입력한 원문 그대로 보존")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        ...
        description="구조화된 요청 목록. 요청이 하나뿐이어도 StructuredRequest 하나를 list에 담는다",
    )
    base_date: str = Field(
        default_factory=current_app_date_iso,
        description="상대 날짜(내일, 다음 주 화요일 등) 해석 기준일. YYYY-MM-DD",
    )
...
def extract_structured_request(text: str) -> StructuredRequest:
    ...
    # function_calling은 tool calling 기반이라 ToolStrategy와 같은 이유로 프록시 환경에서 안전하다.
