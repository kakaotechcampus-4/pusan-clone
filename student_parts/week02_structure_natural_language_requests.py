...
class StructuredRequest(BaseModel):
    ...
    priority: str | None = Field(default="MEDIUM", description="일정 우선순위: HIGH/MEDIUM/LOW")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(default_factory=list, description="요청 개수와 무관하게 StructuredRequest 목록에 담는다")
    # TODO: base_date 필드를 str 타입으로 선언하고 default_factory=current_app_date_iso를 사용하세요.
    base_date: str = Field(default_factory=current_app_date_iso, description="상대 날짜(오늘/내일/다음 주) 해석 기준일(YYYY-MM-DD)")
...
def _coerce_structured_request(value: Any) -> StructuredRequest:
    ...
    # TODO: 예상한 형태가 아니면 RuntimeError를 발생시켜 잘못된 LLM 응답을 조용히 통과시키지 마세요.
        ...
        raise RuntimeError(f"{type(value)} -> StructuredRequest 변환 실패")
...
def week02_system_prompt() -> str:
    ...
    final_answer_rule = "최종 답변은 반드시 StructuredRequestBatch 구조의 structured_response로 반환한다."
    # TODO: StructuredRequestBatch에는 요청이 하나뿐이어도 requests 목록에 StructuredRequest 하나를 담도록 지시하세요.
    single_request_rule = "요청이 하나뿐이어도 requests 목록에 StructuredRequest 하나를 담는다"
    # TODO: personal_create_schedule tool 결과 JSON의 created_schedule을 읽어 필드를 채우도록 지시하세요.
    created_schedule_rule = "개인 일정 생성 요청에서는 personal_create_schedule tool 결과 JSON의 created_schedule을 읽어 필드를 채운다."
    return join_system_prompt([*week02_prompt_parts(), final_answer_rule, single_request_rule, created_schedule_rule])
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        ...
        f"당신은 사용자의 자연어 요청을 구조화하는 agent다. 오늘 날짜는 {current_app_date_iso()}이며 '오늘', '내일', '다음 주 ...' 같은 상대 날짜는 이 날짜를 기준으로 YYYY-MM-DD로 변환한다.",
        # TODO: 자연어를 StructuredRequest 필드(kind/title/date/start_time/end_time/members 등)로 구조화하도록 지시하세요.
        "사용자 요청을 StructuredRequest 필드(kind/title/date/start_time/end_time/members 등)로 구조화한다. 확실하지 않다면 None으로 설정한다.",
