...
StructuredRequestBatch에는 요청이 하나뿐이어도 request 목록에 StructuredRequest 하나를 담는다.
personal_create_schedule tool 결과 JSON의 created_schedule을 읽어 필드를 담는다.
""".strip()
...
class StructuredRequest(BaseModel):
    ...
    end_time: str | None = Field(default=None, description="종료 시간입니다. 확실할 때만 HH:MM 형식으로 채웁니다.")
    members: list[str] = Field(default_factory=list, description="참석자/관련 멤버 목록입니다. 모르면 빈 list로 둡니다.")
    priority: str | None = Field(default=None, description="할 일 우선순위입니다. high/medium/low 중 하나로 채우거나 모르면 None으로 둡니다.")
...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt([*week02_prompt_parts(), STRUCTURED_RESPONSE_PROMPT])
