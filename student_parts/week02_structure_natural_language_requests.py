...
class StructuredRequest(BaseModel):
    kind: RequestKind = Field(default="unknown", description="요청종류 입니다.")
    title: str | None = Field(default= None, description= "일정 제목입니다, 없으면 None으로")
    date: str | None = Field(default= None, description= "YYYY-MM-DD 형식 날짜입니다.")
    start_time: str | None = Field(default= None, description="HH:MM 시작시간입니다.")
    end_time: str | None = Field(default=None, description="HH:MM 형식 종료 시간입니다.")
    members: list[str] = Field(default_factory=list, description="멤버 목록입니다.")
    priority: str | None = Field(default=None, description="할 일의 우선순위입니다.")
    reason: str | None = Field(default=None, description="이 요청으로 판단한 근거입니다.")
    original_text: str = Field(default="", description="사용자 원문 요청을 그대로 보존합니다.")
...
class StructuredRequestBatch(BaseModel):
    ...
    base_date: str = Field(default_factory=current_app_date_iso, description="현재 날짜 기준")
...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt(
        [
            *week02_prompt_parts(),
            "최종 답변은 StructuredRequestBatch 형식으로 반환한다. "
            "요청이 하나뿐이어도 requests 목록에 StructuredRequest 하나를 담는다. "
            "personal_create_schedule 같은 tool을 호출한 경우, 결과 JSON의 created_schedule을 읽어 "
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        f"너는 자연어 요청을 구조화하는 Week 2 agent다. 오늘 날짜는 {current_app_date_iso()}이며 "
        "'내일', '다음주 화요일' 같은 상대 날짜는 이 기준일로 계산해 YYYY-MM-DD로 변환한다.",
        "사용자의 자연어를 StructuredRequest 필드(kind/title/date/start_time/end_time/members 등)로 구조화한다. "
        "확실하지 않은 값은 억지로 만들지 말고 None 또는 빈 list로 둔다.",
        "Week 1 tool 결과 JSON을 이미 받은 경우 tool을 다시 호출하지 말고, 그 payload를 읽어 structured_response로 만든다.",
