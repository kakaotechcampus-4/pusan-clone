...
class StructuredRequest(BaseModel):
    ...
    #기존 description의 무엇을 채우는 필드인지 설명하는 것을 어떤 형식과 값 범위로 채워야하는지 명시하는 것을 추가
    kind : RequestKind = Field(description="요청 종류를 나타내며, personal_schedule, group_schedule, todo, reminder, unknown 중 하나입니다.")
    title: str | None = Field(default=None, description="일정 또는 할 일의 제목입니다. 확실하지 않으면 null로 둡니다.")
    date: str | None = Field(
        default=None,
        description="일정 날짜입니다. 반드시 YYYY-MM-DD 형식의 문자열로 채우고, 확실하지 않으면 null로 둡니다.",
    )
    start_time: str | None = Field(
        default=None,
        description="일정 시작 시간입니다. 반드시 24시간제 HH:MM 형식의 문자열로 채우고, 확실하지 않으면 null로 둡니다.",
    )
    end_time: str | None = Field(
        default=None,
        description="일정 종료 시간입니다. 반드시 24시간제 HH:MM 형식의 문자열로 채우고, 확실하지 않으면 null로 둡니다.",
    )
    members: list[str] = Field(
        default_factory=list, description="참석자/관련 멤버 이름 목록입니다. 알 수 없으면 빈 리스트로 둡니다."
    )
    priority: str | None = Field(
        default=None,
        description="할 일의 우선순위입니다. 높음/보통/낮음 중 하나로 판단되면 채우고, 확실하지 않으면 null로 둡니다.",
    )
    reason: str | None = Field(
        default=None, description="kind와 다른 필드 값을 이렇게 판단한 근거를 한국어 한 문장으로 적습니다."
    )
    original_text: str = Field(default="", description="사용자가 입력한 원문을 그대로 보존한 문자열입니다.")
...
def _coerce_structured_request(value: Any) -> StructuredRequest:
    ...
    raise RuntimeError(f"예상하지 못한 structured output 형태입니다: {type(value)}")
...
def extract_structured_request(text: str) -> StructuredRequest:
    ...
    structed_llm = chat_model().with_structured_output(StructuredRequest, method="function_calling")
    result = structed_llm.invoke(
        [
...
def extract_schedule_request(query: str) -> str:
    ...
    structed_request = extract_structured_request(query)
    return json.dumps(
        {
            "ok": True,
            "tool_name" : "extract_schedule_request",
            "base_date": current_app_date_iso(),
            "structured_request": structed_request.model_dump()
        },
        ensure_ascii=False,
    )
    ...
    return join_system_prompt(
        [
            *week02_prompt_parts(),
            ...
            "personal_create_schedule tool을 호출했다면, 그 결과 JSON의 created_schedule을 다음과 같이 "
            "StructuredRequest 필드에 그대로 매핑하세요: created_schedule.title -> title, "
            "created_schedule.date -> date, created_schedule.start_time -> start_time, "
            "created_schedule.end_time -> end_time, created_schedule.attendees -> members.",
            "created_schedule.end_time 값이 '미정'이거나 HH:MM 형식이 아니면, 그 문자열을 그대로 복사하지 말고 "
            "end_time을 null로 채우세요. start_time도 확실한 HH:MM 값이 아니면 null로 채우세요.",
        ]
    )
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        ...
        "이미 Week 1 tool 결과 JSON(created_schedule 등)을 받았다면 다시 tool을 호출하지 말고,그 payload를 읽어 structured_response 필드를 채우세요.",
