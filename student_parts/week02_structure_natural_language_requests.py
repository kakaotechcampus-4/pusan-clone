...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(default_factory=list, description="여러 자연어 요청을 StructuredRequest 목록으로 나타내는 필드입니다.")
    base_date: str = Field(default_factory=current_app_date_iso, description="상대 날짜 해석 기준일을 나타내는 필드입니다.")
...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt([*week02_prompt_parts(), "최종 답변은 StructuredRequestBatch 형식의 structured_response로 반환해야 합니다."])
